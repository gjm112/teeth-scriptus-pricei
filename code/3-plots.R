...
for (tooth in toothtypes) {
  for (spec in species) {
    first <- read.csv(paste0("~/Documents/shape/data/plots/csv/", tooth, "_", spec, ".csv"), header = FALSE)
    second <- read.csv(paste0("~/Documents/shape/data/plots/csv/", tooth, "_", spec, ".csv"), header = FALSE)
...
for (tooth in toothtypes) {
  file_path <- paste0("/Users/nastaranghorbani/Documents/shape/data/matlab/out_beta_", tooth, "_combined.csv")
